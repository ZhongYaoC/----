...
> 其实静态内存就是目标文件中的数据段！所以要和堆、栈的内存区域分开
...
> 此处全局变量之所以会初始化为0，涉及到编译链接部分；因为全局变量可能涉及到跨模块调用，所以全局变量是在符号表中，未初始化的全局变量被放入BSS段，且默认填入0，而局部变量（非static）是在栈内分配的内存，也不会默认填入0
>
> **参见《程序员的自我修养》读书笔记**
...
核心即为对象允许拷贝，且拷贝后的对象与原对象脱离关系，C语言中全部都是值语义
...
多数的class都是对象语义，如线程，拷贝一个线程后，不能说又出现了一个一模一样的线程；所以除少数值语义class外，**对象语义的class最好禁止拷贝构造函数和赋值运算符，防止无意中破坏程序行为**，如effective C++所言，继承一个禁止了拷贝和赋值运算符的基类noncopyable最佳。
