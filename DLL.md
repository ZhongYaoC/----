...
链接根据使用方式又分为两类，静态链接和动态链接，**静态链接**是把需要的目标代码从库文件（一个或多个目标文件的组合）**拷贝**到最终可执行文件中，此方式下，可执行文件中对外部符号及定义的链接发生在生成可执行文件时（注：拷贝到最终可执行文件中的只是那些需要的函数代码--开启函数级别链接的前提下），执行程序时，不再需要从库中调用目标代码。而**动态链接**，在链接时只是在最终可执行文件中记录下共享对象的名字及其他信息，执行时，**动态链接库的全部内容**将映射到运行对应进程的虚地址空间（注：此处之前有理解错误，认为静态会把所有函数拷贝过去，而动态只运行时映射所需函数，完全反了）。

> 在写下这段文字时， 对于静态和动态链接的认识十分浅薄，上述描述也对也不对，例如拷贝，其实是在做相似段的合并；动态链接库也不是必须对所有函数都做重定位，如果开启延迟绑定，只有在函数被调用时才会做重定位。
>
> 综上，希望之后再翻阅此段文字复习时，希望粗读之后跳转到《程序员的自我修养读后笔记》再去细细品读

（静动态链接优缺点）

（理解完全错误）~~静态链接不管函数是否需要调用都会拷贝进去，导致最后生成的应用程序执行文件偏大，占用较大内存；而动态链接是在需要时，才调用函数，并且可以被多个应用软件同时调用，进一步减少了占用内存大小。~~

静态链接（1）由于会将所需函数单独拷贝到可执行文件中，那么对于每一个调用了这些函数的程序，都会有这些代码的拷贝，重复的代码消耗磁盘空间；（2）运行时各进程会单独在自己的地址空间装入这些函数代码，多个进程调用同一函数时内存出现**多份重复拷贝，占用内存空间**；（3）由于对符号的引用是在链接时完成，所以当库文件中函数发生变化，需要**重新链接**到使用库的应用程序；归根结底，不是静态库本身链接拷贝进去的过大，而是在于没有共享性，从而冗余导致占用过大内存。

> 静态库会对语法做检查？和头文件中比对；而动态库只是简单的合到一起

动态链接又分为两类，隐式的动态链接和显式的动态链接，两者区别在于后者是调用动态链接器所提供的API，来加载对应dll，然后再对应使用的代码段中获取该dll中需要的函数，传入参数，执行完后卸载相应dll；前者需要dll和使用dll的程序间需要一个dll文件中.lib的静态链接。隐式稍后细讲，dll文件本身稍后细讲。
...
```C++
//《程序员的自我修养》静态链接-链接过程控制
//win下的.def文件是控制脚本文件，称为模块定义文件
EXPORTS
    EXPORTSFUNCTION @2
```
...
>  如果dll b中使用与dll a中相同的函数名，那么因为调用顺序为先调用b，然后调用a，所以b的函数会覆盖a中的同名函数--即全局符号介入Global Symbol Interpose。（最终原因在于动态链接时会根据依赖关系逐一遍历dll，然后将其装载）



~~考虑如下，由于a b两个dll中都使用了extern “C”，所以编译器中符号名出现了重名。不过，调整了a、b中参数类型也无法解决？？？~~

> 在实际编译过程中，函数名在对象文件中表现为符号名；而C++引入了函数重载，为防止编译器中出现函数重名导致无法明确使用哪个函数，所以编译器会采用一定的规则将函数名修改变成对应的符号名，但该规则不统一，不同编译器采用不同的修改规则；
>
> 当动态调用时，程序可能会调用别人的dll包，而这个包编译时与该应用程序所用编译器可能不同，调用函数时找不到对应位置，从而调用失败（ABI问题）。而extern “C” 即表示以下函数在编译时不修改符号名，符号名即函数名（其实是遵循C语言符号修饰规范，而Linux下C语言的修饰规范就是不修饰，Windows下MSVC编译器和GCC的Windows版貌似是加上`_`前缀）



##### Q：unix环境下的动态链接库

**Linux中采用ELF格式存储目标文件和可执行文件，而ELF中默认将全局函数和变量视为导出，可以被其他模块所使用。Windows中需要显式的声明导出函数，否则编译器默认不导出，其中`__declspec(export)`表示从本DLL导出的符号，`__declspec(import)`表示从其他DLL导入的符号，Windows下需要显式的声明某符号为导入符号，Linux下不需要额外的声明。**
...
装载库搜索顺序：先搜索LD_LIBRARY_PATH中分号前路径，接着-L指定目录，再LD_LIBRARY_PATH中分号后路径，最后搜索系统lib库路径/usr/lib和由/etc/ld.so.conf配置文件指定的目录

> 实际再搜索/usr/lib和由/etc/ld.so.conf配置文件指定的目录之前，会去搜索ldcofig程序建立的SO-NAME缓存/etc/ld.so.cache，其文件结构更特殊，更适合查找
